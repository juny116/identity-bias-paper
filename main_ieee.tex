...
\subsection{Reference-Provided Control Localizes the Confound}
\label{sec:results-reference}

As a scoping diagnostic, we reran AIME and GPQA with the gold final answer provided to the judge. The prompt asked only whether the candidate final answer matched the provided answer; it did not ask for reasoning-validity judgments. This keeps the control aligned with the available deterministic label and tests whether the interface effect remains when reference-free verification is reduced to answer matching.

Providing the reference answer largely collapses the interface spread. On the matched AIME+GPQA subset and three-interface comparison, the mean interface range drops from 9.4 points in the reference-free no-think condition to 1.6 points with the reference provided. The mean verdict-flip rate drops from 17.1\% to 1.7\% (Table~\ref{tab:reference-control}). Gemma-4-no-think is completely stable in this control: its interface range is 0.0 points and its verdict-flip rate is 0.0\% on both AIME and GPQA. gpt-oss-no-think retains only a small residual instability, with 1.1--1.2 point interface ranges and 3.0--5.6\% verdict flips. qwen3-no-think is nearly flat on AIME, with a 1.2 point range and 1.6\% flips, while GPQA remains the largest residual cell with a 5.8 point range but 0.0\% flips.

The control therefore localizes the main confound. When correctness is reduced to reference-provided final-answer matching, ordinary output-interface choices barely move the measurement. The larger instability in the main experiment lives in reference-free verifier-style judging, where the judge must evaluate the candidate solution without seeing the gold answer. This does not weaken the main result; it scopes it to the critique, best-of-N, and self-repair settings where such reference-free verification is the actual use case.
...
\begin{tabular}{lrr}
\toprule
Condition & Mean range & Mean flips \\
\midrule
Reference-free & 9.4 & 17.1 \\
Reference-provided & 1.6 & 1.7 \\
\bottomrule
\end{tabular}
\caption{Reference-provided control summary from ifc\_reference\_control.csv. Mean range is the mean max-min balanced-accuracy spread across judges and datasets on AIME and GPQA with three matched interfaces. Mean flips is the mean percentage of fixed candidates whose verdict changes across those interfaces.}
\label{tab:reference-control}
\end{table}

\subsection{Deliberation-in-Schema Stabilizes the Comparison}
\label{sec:results-deliberation}

The secondary robustness experiment asks whether adding an explicit deliberation slot to the constrained output contract stabilizes judge comparisons. This is not a native chain-of-thought comparison. The intervention is itself an interface change: a required free-text reasoning field is placed before the verdict fields, and the judge is instructed to reason before giving the verdict. The full deliberation matrix mirrors the no-deliberation matrix: three judges, four datasets, five interfaces, and 4500 deliberation records.

Across the full matrix, deliberation-in-schema shrinks the mean interface range from 8.0 points to 4.9 points (Figure~\ref{fig:ifc-think}). The reduction appears in every dataset: AIME falls from 11.6 to 5.0 points, GPQA from 9.0 to 5.6 points, Olympiad from 6.0 to 5.2 points, and MMLU-Pro from 5.6 to 3.6 points. Ranking stability changes even more clearly. Without the reasoning field, interface-specific judge rankings are unstable in all four datasets; with the reasoning field, the ranking is stable across interfaces in all four datasets. Thus deliberation-in-schema stabilizes measured judge comparisons in this matrix, while remaining an output-interface intervention rather than evidence that private reasoning ability generally solves judge brittleness.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_ifc_think.png}
\caption{Deliberation-in-schema robustness ablation on the full matrix. Bars show the max-min balanced-accuracy range across five interfaces for each judge and dataset. Adding a required leading reasoning field reduces interface range in every dataset and stabilizes the interface-specific judge rankings in all four datasets.}
\label{fig:ifc-think}
\end{figure}
...
\begin{tabular}{llrr}
\toprule
Condition & Dataset & Mean range & Rankings \\
\midrule
No deliberation & AIME & 11.6 & 2 \\
No deliberation & GPQA & 9.0 & 2 \\
No deliberation & Olympiad & 6.0 & 3 \\
No deliberation & MMLU-Pro & 5.6 & 2 \\
Deliberation & AIME & 5.0 & 1 \\
Deliberation & GPQA & 5.6 & 1 \\
Deliberation & Olympiad & 5.2 & 1 \\
Deliberation & MMLU-Pro & 3.6 & 1 \\
\bottomrule
\end{tabular}
}
\caption{Deliberation-in-schema robustness summary from ifc\_think\_vs\_nothink\_full.csv. Mean range is the mean across judges of the max-min balanced-accuracy spread across five interfaces. Rankings is the number of distinct interface-specific judge rankings observed in the dataset condition.}
\label{tab:deliberation}
\end{table}

The practical reading remains narrow. Adding a leading deliberation field stabilizes the measured comparison in this matrix, but it should not be described as proof that model reasoning ability solves judge brittleness. It is a controlled output-interface intervention, and its success reinforces the main thesis: the judge interface changes the measurement.

\subsection{Subjective Pairwise Judging Shows Smaller but Real Interface Drift}
\label{sec:results-subjective}

The objective experiments provide measurable correctness, but many LLM-as-judge deployments are used precisely when no deterministic gold answer exists. We therefore ran a deployment-validity diagnostic on 150 RewardBench preference pairs from chat, chat-hard, and safety categories, excluding code and math. The task is pairwise A-vs-B preference judging with randomized A/B order. The five interfaces mirror the objective design at the level of output contract: choice-only, rubric-then-choice, score-then-choice, choice-then-feedback, and feedback-then-choice. The primary metric is label-free preference instability: whether the same judge changes its A/B choice on the same pair across interfaces. Agreement with the curated chosen side is reported only as a secondary preference-alignment diagnostic, not as deterministic accuracy.

The subjective extension shows that the confound is not confined to objective verification, but it is smaller and more judge-dependent in this setting (Table~\ref{tab:subjective-control}). Interface preference-flip rates are 6.0\% for Gemma-4-no-think, 16.0\% for gpt-oss-no-think, and 18.0\% for qwen3-no-think. The order-swap control shows that position bias is a major co-factor: position-driven flips are 4.0\%, 13.4\%, and 10.7\%, respectively. After subtracting this control estimate, the interface drift beyond position bias is +2.0 points for Gemma-4-no-think, +2.6 points for gpt-oss-no-think, and +7.3 points for qwen3-no-think. The range in curated chosen-side agreement across interfaces follows the same pattern: 2.0 points for Gemma-4-no-think, 3.4 points for gpt-oss-no-think, and 8.6 points for qwen3-no-think.

Thus benign output interfaces also shift real subjective preferences, especially for the weakest judge, but the effect should be read honestly. In this RewardBench diagnostic, interface-induced subjective preference drift is not as large as the objective verifier instability, and for gpt-oss-no-think much of the raw flip rate is entangled with A/B position sensitivity. The result supports deployment validity for the interface-confound claim while showing that position bias and judge strength must be separated from interface effects in no-gold judging.
...
\begin{tabular}{lrrrr}
\toprule
Judge & Interface flips & Position flips & Beyond position & Chosen range \\
\midrule
Gemma-4 & 6.0 & 4.0 & +2.0 & 2.0 \\
gpt-oss & 16.0 & 13.4 & +2.6 & 3.4 \\
qwen3 & 18.0 & 10.7 & +7.3 & 8.6 \\
\bottomrule
\end{tabular}
}
\caption{Subjective no-gold RewardBench diagnostic from ifc\_subjective\_control.csv. Interface flips are preference changes across five pairwise output interfaces on fixed A/B pairs. Position flips are estimated from order-swapped controls on choice-only and feedback-then-choice. Beyond position is the difference between the two. Chosen range is the max-min range in agreement with the curated chosen side across interfaces, reported as secondary preference-alignment evidence rather than deterministic accuracy. Values are percentages or percentage points.}
\label{tab:subjective-control}
\end{table}
...
The reference-provided control further narrows the interpretation. When judges are shown the gold final answer and asked only to check final-answer match, the interface range and verdict-flip rate largely collapse. The measured instability is therefore not a claim that every possible grading setup is equally brittle. It is a claim about reference-free verifier-style judging, where the judge must evaluate a candidate solution without being handed the answer.

The RewardBench diagnostic adds the complementary deployment point. In no-gold pairwise preference judging, interface changes also move decisions beyond plain A/B order effects, but the magnitude is smaller and more judge-dependent than in objective verification. This supports the broader measurement claim without turning subjective preference into an accuracy experiment.
...
The deliberation-in-schema ablation is best read narrowly. Adding a required reasoning field before the verdict stabilizes the tested comparisons across all four task families and reduces the mean interface range in each dataset. That is useful evidence that some interface choices can reduce instability. It is not evidence that private reasoning ability generally solves judge brittleness. The intervention changes the output contract by forcing an explicit deliberation slot before the verdict, so it is itself part of the interface design space.
...
The reference-provided control also shows that the main effect is not expected to transfer unchanged to reference-based final-answer checking. When the answer is given, the output interface becomes much less consequential in our AIME and GPQA control. The paper's strongest scope is therefore reference-free verification, not every form of LLM-assisted grading.

Third, the subjective extension uses curated preference labels only as secondary evidence. These labels are useful for checking whether interface changes move agreement with the preferred response, but they are not deterministic ground truth. The main subjective metric is therefore decision instability under fixed response pairs. Position bias is also a co-factor, and the order-swap control shows that it accounts for a substantial part of the raw flip rate for some judges.

Fourth, the candidate pool is small by benchmark-leaderboard standards: 75 sampled candidate solutions per dataset before uniform context-overflow exclusions in the objective runs and 150 RewardBench pairs in the subjective diagnostic. The design gains paired control because every judge-interface cell sees the same candidates, but larger samples would give sharper estimates of pairwise gap changes and task-specific rank instability.

Fifth, the interface variants are representative rather than exhaustive. We test verdict-only, rubric-verdict, score-then-verdict, verdict-plus-repair, and repair-then-verdict, along with deliberation-in-schema versions of the same interface matrix. In subjective pairwise judging, these map to choice-only, rubric-then-choice, score-then-choice, choice-then-feedback, and feedback-then-choice. Other schemas, rubric granularities, confidence formats, and constrained-decoding implementations may produce different magnitudes. The paper's claim is therefore about interface dependence and comparison instability, not about a universal ordering of all possible interfaces.

Sixth, constrained decoding reduces literal JSON failures but does not eliminate all non-verdicts. Long candidate solutions create uniform context-overflow exclusions, and some judge-interface cells have lower valid-verdict coverage because of truncation. We report coverage so that verdict effects are not confused with missingness, but larger-context runs would reduce this nuisance layer.

Finally, deliberation-in-schema is still a design ablation, not a universal mitigation. It stabilizes the tested local-judge comparisons across four datasets, but broader model coverage and subjective no-gold judging are needed before treating it as a general recommendation.


\section{Conclusion}
\label{sec:conclusion}

LLM judge reliability cannot be read from the judge model name alone. Holding judged content fixed under constrained decoding, this study shows that benign output-interface choices can change pairwise judge gaps, adjacent rankings, and item-level verdicts in reference-free verification, and can also shift no-gold subjective preferences beyond plain position bias. The direction of the interaction depends on the task family, so pooled summaries can hide the instability rather than resolve it. The practical conclusion is modest but important: an LLM judge result is a measurement produced by a model, a task distribution, and an output interface together. Reports that compare judge models should therefore specify the interface and test whether the comparison is stable under reasonable interface variants.
