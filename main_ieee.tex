...
\begin{abstract}
Large language models (LLMs) are increasingly used as judges for benchmark construction, model selection, and critique-repair pipelines. Such evaluations usually specify the judge model, but they often treat the judge-side interface as an implementation detail. This paper studies a narrower measurement confound: benign output-interface choices, including schema shape, field order, rubric structure, score-versus-verdict order, and repair coupling. Holding judged content fixed and using constrained decoding, we ask whether these choices interact differently with different judge models, so that model-comparison conclusions depend on the interface. This is not the familiar claim that prompts matter in general; it is a claim about the judge-model by interface interaction that can affect the apparent leaderboard. Repair coupling is a particularly important case because a system may ask for a verdict and a correction in the same call, even though those are distinct tasks. We further use parser-aware replay as a methodological safeguard, separating behavioral verdict changes from measurement artifacts when structured outputs fail. The resulting recommendation is procedural: LLM-as-judge reports should standardize and disclose the judge interface, treat output handling as part of the evaluation protocol, and avoid comparing judge models under an unspecified or single unexamined interface.
\end{abstract}

\section{Introduction}
\label{sec:intro}

Large language models are now routinely used to evaluate other model outputs. They grade benchmark answers, select among system variants, provide feedback for refinement pipelines, and stand in for human preference annotators when human evaluation is too slow or expensive. In these settings, the judge model is usually named and compared, while the judge-side interface is treated as a prompt-engineering detail. A leaderboard may report that judge model A is more reliable than judge model B under one rubric, output schema, or scoring protocol, and leave open whether the same conclusion would hold under another benign interface.
...
All judges are served through the repository's local model presets. qwen3-no-think uses \texttt{Qwen/Qwen3-32B} with \texttt{enable\_thinking=False}; gpt-oss-no-think uses \texttt{openai/gpt-oss-20b} with \texttt{reasoning\_effort=low}; Gemma-4-no-think uses the Gemma-4 local serving configuration. The interface-confound runs use constrained JSON decoding through \texttt{response\_format=json\_schema} with strict schemas and a judge completion budget of \texttt{max\_tokens=16000}. The default endpoint is \texttt{http://localhost:8000/v1}.

The secondary deliberation-in-schema run uses the same three judge families on AIME and GPQA. It does not rely on native private chain-of-thought modes. Instead, the constrained schema adds a required leading free-text \texttt{reasoning} field before the verdict fields, paired with an instruction to think step by step before emitting the verdict. This makes deliberation an explicit output-interface intervention.
...
All pairwise judge comparisons are item-paired within an interface and dataset. We report absolute balanced-accuracy gaps in percentage points with cluster-bootstrap 95\% confidence intervals, resampling problem identifiers and retaining all candidate rows associated with a resampled problem. This clustering handles the small number of cases where the same problem appears with both a wrong and a correct candidate solution.

For model-based inference, we fit generalized estimating equation (GEE) binomial logistic regressions for verdict correctness. The per-dataset models include judge, interface, and judge-by-interface terms and cluster by problem identifier. We also fit a pooled two-way model with dataset fixed effects and judge-by-interface terms. Finally, to test whether the judge-by-interface interaction changes across task families, we fit a pooled three-way model with judge, interface, dataset, and judge-by-interface-by-dataset terms, clustering by dataset-specific problem identifier. The joint robust Wald test on the 24 three-way interaction terms is the pre-specified test for task-dependent interface confounding. The frozen GEE summary is stored in the paper's tables directory, and the analysis is generated by the interface-confound, item-disagreement, and think-versus-no-think scripts.
...
\caption{Per-dataset interface instability summary. ``GEE terms'' counts significant judge-by-interface terms in the per-dataset GEE model of verdict correctness, as frozen in \texttt{paper/tables/ifc\_gee\_summary.csv}. Kendall $\tau$ is the minimum rank correlation between any two interface-specific judge rankings within the dataset. The analyzable pool removes only uniform context-overflow exclusions before judge-specific non-verdicts.}
